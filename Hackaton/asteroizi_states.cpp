...
void AsteroiziState::InitState() {
	...
	addCelestialBody({ 0,0, 0 }, { 0,0,0 }, 10000, 1.7, "texture_sun");
	
	for (int i = 0; i < 100; i++) {
		Vector3 pos = { GetRandomValue(-20,20),GetRandomValue(-20,20), GetRandomValue(-20,20) };
		Vector3 vel = { GetRandomValue(-5, 5), GetRandomValue(-5, 5), GetRandomValue(-5, 5) };
		addCelestialBody(pos, vel, 1, 0.5, "texture_mercur");
	}
